...
def filter_group_data_simplified(data_dict: dict, language: str) -> dict:
    df = pd.DataFrame(
        data_dict["report"]["jobs"][0]["validationResult"]["details"]["ruleSummaries"]
    )
    # df = df[df["clause"] != "7.21.4.2"]
    # df = df[df["clause"] != "7.18.5"]
    ...
    print(df_grouped_data.columns)
    # exit(0)
    df_grouped_data = df_grouped_data.groupby(["clause", "testNumber"], as_index=False).agg(
